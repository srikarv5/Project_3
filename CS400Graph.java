...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {     
    	...
    	// add the startint path to Priority Queue
    	if(! this.containsVertex(start)) throw new NoSuchElementException();//if start can't be found
    	if(! this.containsVertex(end)) throw new NoSuchElementException();//if end can't be found
    	paths.add(new Path(vertices.get(start)));
    	for(Edge e : paths.peek().end.edgesLeaving) {
    		Path thisPath = new Path(paths.peek(),e);//This is the path
    		if(thisPath.end.data.equals(end)) {
    			return thisPath;
    		}  		
    		//add all the frontier nodes to the priority queue
    		paths.add(new Path(paths.peek(),e));
    	}
    	visited.put(start, start);//add the vertex to visited
    	paths.remove();//remove the node since the more completet path is already in pq

    	while(!paths.isEmpty()) {
    		//start with the vertex that have the smallest weight
    		Path thisPath = paths.peek();

    		for(Edge e : paths.peek().end.edgesLeaving) {
    			
        		//add to priority queue the vertex connected to this vertex
    			if(!visited.contains(e.target)) {
    				// if the pq contain connecting node, should not be add to pq
    				//add this path to frontier
    				paths.add(new Path(thisPath, e));
    			}
        	}
    		if(thisPath.end.data.equals(end))
    			return thisPath;
    		visited.put(paths.peek().end.data, paths.peek().end.data);
    		paths.remove();
    	} 	
    ...
    public static void main(String[] args) {
    	...
    	graph.insertEdge("D","Z", 1);
    	graph.insertEdge("D","Y", 2);
    	graph.insertEdge("D","X", 3);
    	graph.insertEdge("Z","S", 7);
    	graph.insertEdge("Y","S", 5);
    	graph.insertEdge("X","S", 5);
    	
    	System.out.println("Shortest Path: " + graph.shortestPath("A", "S"));
    	System.out.println("Shortest Distance: " + graph.getPathCost("A", "S"));
